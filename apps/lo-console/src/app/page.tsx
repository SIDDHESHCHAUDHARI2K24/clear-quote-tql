...
type StaffUser = components["schemas"]["StaffUserOut"];

type SessionState = { kind: "loading" } | { kind: "signed-in"; user: StaffUser };

// Placeholder signed-in home (P5 replaces this with the real dashboard).
// `src/middleware.ts` already redirects here-to-/login when the session
// cookie is missing; this page's own /me call is the second, authoritative
// check — a present-but-expired/revoked cookie only middleware can't see.
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-4 p-8">
      ...
      <p role="status">
        {state.kind === "loading"
          ? "Checking session…"
          : `Signed in as ${state.user.full_name} (${ROLE_LABELS[state.user.role]})`}
      </p>
