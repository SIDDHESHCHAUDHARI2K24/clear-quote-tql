...
describe("ClientsFilterBar", () => {
  ...
  it("toggles the active-only control", async () => {
    const onChange = vi.fn();
    render(
      <ClientsFilterBar
        filters={DEFAULT_FILTERS}
    ...
    await userEvent.click(screen.getByRole("radio", { name: "Active only" }));
    expect(onChange).toHaveBeenCalledWith({ hasActive: "true" });
  });
