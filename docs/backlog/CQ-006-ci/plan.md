...
| 15 | Decision | G3 — CI must run `pytest seed` (CQ-010) | Read `seed/tests/*`: everything except `seed/tests/test_documents_watermarked.py` only needs Postgres (already a service container in this job, `cq_test`) and env already set (`APP_ENV`, `SECRET_KEY`→`FIELD_ENCRYPTION_KEY` via `backend/conftest.py`'s `setdefault`, `SEED_STAFF_PASSWORD` via `seed/tests/conftest.py`'s `setdefault`). `test_documents_watermarked.py` does a real MinIO upload/download round-trip via `boto3`/`get_settings()`'s `S3_*` fields — no mocking, by design (AC6 proves a real object exists). GitHub Actions `services:` containers can't be given a run command, and `minio/minio`'s default `CMD` doesn't start the server without one (`infra/docker-compose.yml` sets `command: server /data ...` explicitly) — used a plain `docker run` step instead, same image/credentials as local infra, health-polled via `curl .../minio/health/live` before proceeding. Added a `Create MinIO buckets` step (`boto3`, already a main dependency) to create `clear-quote` and `clearquote-demo-docs` (mirrors `minio-init` + `ensure_demo_docs_bucket`'s local-dev roles, since there's no compose stack in CI to run them). Switched the job's `S3_*` env from dummy/unresolvable values to the real MinIO container's credentials (`cq-minio`/`cq-minio-secret`, matching `infra/docker-compose.yml`) — `VALKEY_URL`/`TEMPORAL_*` stay dummy, untouched (still only exercised via monkeypatching/fakeredis, never `pytest seed` for real). Added `- run: uv run pytest seed` as the job's last backend step, deliberately the same command text as `make test`'s second line, so the two can't drift (per the orchestrator's brief). |
...
Verified locally end to end (this worktree, against the shared `clear-quote` stack's real Postgres/MinIO, with the job's exact env var values): `uv run pytest backend` (214 passed), `uv run pytest seed` (23 passed), `make lint` and `make test` both green from repo root. See `post-dev.md`'s "CI clean-up pass" section for full evidence and the real CI run once pushed.
