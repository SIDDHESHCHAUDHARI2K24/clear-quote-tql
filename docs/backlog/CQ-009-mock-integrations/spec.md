...
**Call log** (`logging.py`, `models.py`): `async def record_call(adapter: str, request_summary: dict, success: bool, latency_ms: int, error_code: str | None = None, application_id: uuid.UUID | None = None) -> None` inserts one `integration_calls` row (schema in CQ-007). No Valkey cache — CQ-029 reads the latest row per adapter with SQL.
