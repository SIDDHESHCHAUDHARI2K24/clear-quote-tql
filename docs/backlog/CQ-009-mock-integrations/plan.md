...
| 2 | Decision | `record_call` / `Mock*.__init__` session handling | Every `Mock*` class takes `session: AsyncSession` at construction (Protocols only constrain the async business methods, spec.md's method-signature table, not `__init__`), and `common.logging.record_call` takes a leading `session: AsyncSession` parameter not shown in spec.md's shorthand table. Reason: CQ-004's `app.core.db` module-level `engine`/`AsyncSessionLocal` is permanently bound to `Settings.database_url` — a different database from `TEST_DATABASE_URL`, and one CI's Postgres service (`.github/workflows/ci.yml`) never creates (only `cq_test` exists there). Every mock call, success or failure, calls `record_call` on its way out, so if it opened its own session against `database_url` the whole suite (not just CQ-009's tests) would fail in CI. Passing the caller's session through instead means the call-log row lands in the same session/transaction the caller (and `backend/conftest.py`'s `db_session` fixture) is already using. Downstream (CQ-011, CQ-013) construct `MockXClient(session)` from whatever session they already hold. Note: `record_call` calls `session.commit()`, so constructing a `Mock*` with a session that has other pending uncommitted work will commit that work too — acceptable for a fire-and-forget audit log, but worth knowing for CQ-011/CQ-013. |
