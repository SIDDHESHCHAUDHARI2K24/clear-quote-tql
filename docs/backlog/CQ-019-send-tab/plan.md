...
| 3 | Decision | AC2 parity | Decided: one function, `app.features.quotes.send.view_model.build_package_view_model(db, package, *, recommendation_text, lo_note, as_of, expires_at, expired=False, superseded=False)`, holds the Quote→`ReportInputs` mapping, the CQ-021 builder call and CQ-023's `compute_matches_for_package`. `freeze_package_version` calls it with the send time; `GET /packages/{id}/report` calls it with the live draft and "now". The contract test allows only `header.prepared_at`, `header.rates_as_of`\* , `header.expires_at`, `header.expired`, `header.superseded` to differ. \*`rates_as_of` is set from the same `as_of` as `prepared_at` in both paths, so it is equal whenever both run on the same day; it is listed because it is a send-time field too. |
...
| 9 | Decision | Stale rule for readiness | Decided: a selected quote is stale when `quote.stale` OR `priced_at` is older than 21 days (catalog §12 `is_rate_stale`). Grace Kim (priced 25 days ago in the seed) is blocked with "Quotes are out of date" → Pricing. |
...
| 14 | Decision | Portal URL placeholder | Decided: `render_letter_html(..., portal_url=None)`; `None` renders `settings.portal_base_url` + `/report/(link sent with your quote)` text placeholder. CQ-020 passes the real `/report/{token}` URL. |
