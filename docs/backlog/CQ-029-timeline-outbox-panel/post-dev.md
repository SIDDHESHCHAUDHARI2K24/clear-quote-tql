...
| AC2 — outbox lists every email after demo-reset + one send; opens Marcus Hale's quote email (HTML + PDF) | ✅ | `test_outbox_list_and_detail`, `OutboxDetail.test.tsx`, `test_outbox_attachment_stream` (a fixture PDF uploaded via `core/storage`, live MinIO round trip); manual curl confirms `type=quote_sent` rows (Grace Kim, Luis Romero) after `make demo-reset`. The PDF-download half re-checked (P56 U4, phase-p5-p6-verification.md) now that CQ-020 has merged: `e2e/cross-app/outbox-pdf.spec.ts` sends Marcus Hale's real package from the Send tab, opens his quote email in `/outbox`, asserts the HTML renders in the sandboxed iframe, and downloads `preapproval-letter.pdf` (`content-type: application/pdf`, non-empty `%PDF-` bytes) |
...
- ~~`outbox/service.py`'s subject-based `type` classifier only knows today's OTP/borrower-action/quote-sent subjects. When CQ-020 (real quote-send emails), CQ-024 (letter emails) and CQ-034 (support emails) land, extend `_TYPE_SUBJECT_RULES` with their real subjects, or those emails will keep showing as `other`.~~ CQ-020's half resolved (P56-fix): `_TYPE_SUBJECT_RULES["quote_sent"]` now also matches `email_template.SUBJECT` ("Your pre-approval and numbers from Total Quality Lending"), imported directly so the two can't drift apart again; `test_outbox_real_cq020_subject_is_quote_sent` covers it. CQ-024/CQ-034 subjects still need adding when those land.
- ~~AC2's PDF-download half stays `pending — re-check after CQ-020`~~ Resolved (P56 U4): `e2e/cross-app/outbox-pdf.spec.ts` re-verifies against Marcus Hale, now a real sent persona with a real PDF attachment.
