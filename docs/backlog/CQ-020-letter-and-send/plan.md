...
| 22 | Decision (PR #30 review, major 1) | Double click could double-send: `POST /send` committed `queued` + new id A and released the lock *before* starting A, so a second POST could see `queued`, get NOT_FOUND from Temporal for A and start B; Freeze didn't check which run owned the package | Fixed two ways. (1) `start_send` starts the workflow **while holding the package row lock** and commits the id only after Temporal accepted it, so a POST waiting on the lock always finds a workflow Temporal knows (chosen over a "queued is in flight for 30 s" grace window: no timing guess, and a failed start never leaves a phantom `queued`). (2) Freeze, after locking the package, raises non-retryable `SendSupersededError` when `send_workflow_id` isn't its own id; the workflow then returns `""` without marking anything failed. Tests: `test_concurrent_posts_send_once` (two real connections, a slowed `start_workflow`), `test_superseded_run_is_a_noop` |
| 23 | Decision (PR #30 review, major 2) | A dead send (terminated, cancelled, timed out, never picked up) left `send_status` in flight forever: every PUT 409'd and the UI polled forever | The workflow starts with `execution_timeout` 10 min (`SEND_EXECUTION_TIMEOUT`). `GET /send-status` and `PUT /package` call `reconcile_send` while the send looks in flight: when Temporal says the workflow isn't running (NOT_FOUND or closed) the package becomes `failed` with "The send stopped before finishing. Try again." (Temporal unreachable = still in flight). Both routes get Temporal through the lazy `get_temporal_provider`, so they only connect when a send looks in flight. The workflow catches cancellation and runs `mark_send_failed` with the same message. The Send tab stops polling after 2 minutes with "The send is taking longer than expected…". Tests: `test_send_workflow_has_an_execution_timeout`, `test_status_reports_a_terminated_send_as_failed`, `test_put_after_a_terminated_send_saves`, `test_cancelled_send_is_marked_failed`, Vitest "a send that stays in flight too long…" |
...
| `GET /packages/{id}/send-status` | 200 `SendStatus {package_id, workflow_id, status: idle/queued/rendering/emailing/done/failed, error, version, recipient_email, sent_at}`. A send whose workflow is gone reads `failed` (Decision 23) |
...
| `PUT /applications/{id}/package` | unchanged shape; reopens a sent package (Decision 2); 409 `SEND_IN_PROGRESS` while the send's workflow runs (a dead send is marked failed and the PUT saves, Decision 23) |
