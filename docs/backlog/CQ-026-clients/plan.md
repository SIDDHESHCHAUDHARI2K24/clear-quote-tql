...
| 14 | Fix (review round 1, critical) | Cross-LO data leak in `get_client_detail` | `_application_rows_for_client` returned *every* application on the client regardless of role, so an LO viewing a client shared with another LO saw the other LO's applications, sent versions and activity too -- contradicting `core/auth.py::scope_applications`'s "an LO only ever sees their own applications" (CQ-014). Fixed: `_application_rows_for_client(db, client_id, user)` now runs its query through `scope_applications`, the same helper every other application-scoped route uses. The gate simplified to "no in-scope applications for an LO -> 404" (no separate existence check needed -- `scope_applications` already answers it). Manager/Admin unaffected (still see every application). `test_client_detail_scoping_needs_active_application` rewritten to assert the LO sees only their own application, not the other LO's -- round 1's version did **not** yet assert `sent_versions`/`activity` are scoped too, despite them deriving from the same fixed query; round 2 (decision #18) added those assertions. `test_client_detail_manager_sees_every_lo_application` added for the Manager side (same round-1 gap, closed by round 2 for this test too). |
| 15 | Decision (review round 1, amended round 2) | List-level aggregates (`application_count`/`active_status`/`last_activity`/`has_active`) for an LO | Scoped to the requesting LO's own applications only (`_application_scope_clauses(lo_scope)`, `lo_scope = user.id` when `user.role == LO`), so a shared client's row doesn't reveal another LO's application count, active status or activity to the LO. A Manager/Admin's aggregates stay unscoped even when filtering by `lo_id` -- that filter narrows which clients appear, not what a Manager (who can already see everything) is told about them. Tested: `test_client_list_aggregates_scoped_to_lo_own_applications` (`application_count`, and — added round 2 — `last_activity`'s value and its effect on `-last_activity` ordering), `test_client_list_aggregates_unscoped_for_manager`. Round 1 left `has_active`'s filter predicate (`_client_active_exists`) unscoped, reasoning it was "out of the three named aggregates" -- review round 2 (finding #1, major) corrected this: `has_active` leaks the same way (a client whose only active application is another LO's would wrongly show/hide for the requesting LO). Fixed: `_client_active_exists(lo_scope)` now takes the same `lo_scope`. Tested: `test_client_has_active_filter_scoped_to_lo_own_applications`, `test_client_has_active_filter_unscoped_for_manager`. |
| 16 | Fix (review round 1, minor; signature changed round 2) | `_merged_activity` batching | Called `list_activity` once per application (each re-fetching the client row, each over-fetching up to `limit` per application before a Python re-sort/truncate). Replaced with a new public `applications.timeline.service.list_activity_for_applications(db, client, applications, limit=50)`: one query across every scoped application's events (`ORDER BY at DESC, id DESC LIMIT 50`, matching `list_activity`'s own order) and one batched staff-name lookup, sharing a new `_events_to_out` helper with `list_activity`. Round 1 inferred `client` from `applications[0].client_id` with a docstring assumption ("all `applications` are assumed to share one client"); review round 2 (finding #4) replaced the assumption with an explicit `client` parameter (the caller, `get_client_detail`, already has it) plus an `assert all(a.client_id == client.id for a in applications)`, so a caller bug fails loudly instead of silently mislabeling the borrower name or merging out-of-scope events. Direct unit tests added: `applications/timeline/tests/test_service.py` (merge order, `limit`, empty input, the new assertion). |
...
| 18 | Fix (review round 2) | `test_client_detail_scoping_needs_active_application` / `test_client_detail_manager_sees_every_lo_application` didn't cover `sent_versions`/`activity` | Both tests only asserted on `applications`, even though decision #14's fix scopes `sent_versions` and `activity` too (they derive from the same `_application_rows_for_client` result). Both tests now add a sent `QuotePackageVersion` and an `ActivityEvent` to each of the two applications and assert the LO's response contains only their own version/event ids (Manager: both). |
