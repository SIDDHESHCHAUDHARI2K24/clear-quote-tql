...
| 7 | Decision | Activity section (detail) | `applications/timeline/service.py`'s docstring invites CQ-026 to reuse *the service*: calls the public `list_activity(db, application, page=1, page_size=50)` once per application the client has (small, bounded N — typically 1) and merges/sorts/truncates to 50 in Python, rather than importing its private `describe_event`/`_resolve_actor` helpers. No new backend endpoint. |
