...
| 4 | Decision | Save & AutoQuote semantics (AC3 says "replaces") | Decided: `POST /scenarios/{id}/autoquote` now replaces the scenario's Par/Buydown quotes instead of appending. It first refreshes the scenario's enrichment-owned inputs (tax, insurance, HOA, rent) from `field_values`, prices, and only then deletes the old quotes, so a pricing error leaves them untouched. Manual picks in that scenario are re-computed with the fresh inputs (same product), not deleted. When the replaced quote was recommended, the recommendation moves to the new quote with the same label. |
