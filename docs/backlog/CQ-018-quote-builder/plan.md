...
| 2 | Decision | AC1 buydown risk: no Buydown candidate for DSCR buckets `BELOW_1_00` and `ONE_TO_1_25`, and Marcus's grid had only 2 rows (AC4 needs ≥ 8) | Decided: added 8 bucket-agnostic DSCR rate-sheet rows to `seed/providers/rate_sheet.yaml` (Harborline Capital and Keystone Investor Lending): a ladder from 1.5 points to a 1.5-point credit, with 5-year PPP, FICO ≥ 680 and LTV ≤ 80. **Corrected in PR review round 1:** each price is more than **0.750** from par (the widest bucket-specific par row is Blue Harbor Thin at 100.750 in `BELOW_1_00`), so no bucket's par row changes. The original claim ("more than 0.500") was wrong: Low-Points at 99.375 beat Thin and moved a non-STR `BELOW_1_00` par from 7.875 to 7.500. Low-Points is now 99.125 and the Keystone 45-Day Buydown 99.000. No seeded persona's par changed either way (Marcus is STR, so his `BELOW_1_00` par is Blue Harbor STR); see post-dev.md "PR review round 1" for the per-persona query. The Buydown is 7.375% at 1.000 point. The `8 <= rows <= 15` bound in `seed/tests/test_provider_rows_seeded.py` is now `<= 25`. No persona special-casing in code. |
...
| 14 | Decision (PR review M1) | The ladder's Buydown also lands in every investment persona's "At DSCR 1.00" group, so Luis Romero's seeded package had 4 options with two "Buydown"s (the portal's `getByRole("radio", {name: /buydown/i})` is strict) | Decided: cap the seeded (fixture-layer) package at the recommended quote plus up to 2 alternatives, in group order (`seed/loader.py` `SEEDED_PACKAGE_MAX = 3`), which is CQ-019's default-draft rule. Bucket-scoping the Buydown rows was rejected: Marcus's assumed group is `ONE_TO_1_25`, the same bucket as every other investment persona's assumed group, so AC1's 4 quotes need a Buydown in exactly the bucket Luis and Grace share. Result: Luis gets Par 7.625 (recommended), Buydown 7.375, Par 7.250; Grace gets Par 7.500, Buydown 7.375. |
| 15 | Decision (PR review M2) | Empty grid (investment LTV > 80, primary LTV > 97) returned 500, and a PUT had already committed `stale=True` | Decided: `select_par_and_buydown` raises `NoEligibleProductsError` (422 `no_eligible_products`, "No products at 90% LTV"; also a `ValueError` for old callers). PUT, autoquote, reprice and create price before committing; a failure rolls back everything the request flushed. |
| 16 | Decision (PR review M3) | The grid's "Buydown" tag came from the mock's `is_buydown_rate`, which differs from AutoQuote's rule | Decided: `GET /scenarios/{id}/products` recomputes both tags with `select_par_and_buydown` (`tag_par_and_buydown`). |
| 17 | Decision (PR review minors) | PUT staleness, reprice logging, races, manual pick trust, PUT validation, same-bucket note | Decided: a PUT marks quotes stale (and logs `scenario.updated`) only when the normalized stored inputs change, and the overlay skips the PUT on "Choose manually" when nothing changed. Reprice/autoquote events carry `deleted_quote_ids` and `recommendation_cleared`. Every builder write, the manual pick and the CQ-017 override/revert routes start with `SELECT … FOR UPDATE` on the application row (`applications/locks.py`). A manual pick is matched on `(investor, product, lock)` against a fresh grid and priced from the server row (422 `product_not_offered` otherwise). A primary PUT with `dscr_bucket` or a PPP is a 422 naming the field. The same-bucket note belongs to the pipeline's collapsed group (the earliest `created_at` batch with one scenario), not to "the app has one group". |
