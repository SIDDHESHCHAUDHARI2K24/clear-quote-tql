...
| AC4 — tile link opens the matching filtered list with the matching count | **Pending — re-check after CQ-027.** Today: tile hrefs asserted (unit + live e2e) | `Tiles.test.tsx`; `e2e/lo-console/dashboard-tiles.spec.ts` ("tiles are links with the spec.md query parameters") run live against slot 12 — all 7 hrefs correct, incl. `&lo_id=` composition (`Tiles.test.tsx`'s second case) |
...
| AC7 — react-doctor passes; tiles/lists keyboard navigable | Done | `npx react-doctor -y --blocking error` on `apps/lo-console`: 86/100, 0 errors (2 pre-existing warnings, both in `features/workspace/`, unrelated to this item); every tile/list row is a `next/link` `<a>` (native keyboard focus + activation, no custom key handling needed) |
...
- CQ-027: once the real Applications list exists, re-run AC4's Playwright
  assertion to also check the list shows exactly the tile's count (today
  only the hrefs are asserted).
- CQ-028: once the real re-verify endpoint exists, re-run AC5 through it
  instead of a direct DB update.
- Whichever item wires up CQ-020's quote-package resend path: once a
  resend can happen after OPTION_SELECTED/INQUIRY, `_latest_sent_versions`'
  "highest version wins" pick will read the fresh (unacted-on) version's
  `borrower_action` (`None`) instead of the acted-on one, silently
  reverting the attention-list reason to the generic fallback text
  ("Selected an option" / "Borrower has a question"). Fixing this needs
  picking the *latest version with a non-null `borrower_action`*, not
  simply the latest version -- left as a follow-up since it isn't
  reachable today (code-review finding, see above).
- `apps/lo-console/src/app/(staff)/page.test.tsx` is shared scaffolding
  ...
  land -- expect a small merge conflict there, not a real regression.
