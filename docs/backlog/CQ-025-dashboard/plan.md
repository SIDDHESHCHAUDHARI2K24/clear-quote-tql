...
| 7 | Decision (stage-6 fix, cq-025-fix) | "Pre-approvals sent" tile vs. CQ-027's `sent_or_later` | Original code kept its own `_PRE_APPROVAL_SENT_STATUSES` frozenset that counted *every* `Stale` application, including one that aged out straight from `Priced` without ever being sent (CQ-030 spec.md) — disagreeing with E10/CQ-027's own `sent_or_later` definition ("Stale after a send"). Fixed: the tile now imports and applies `applications.listing.service.build_sent_or_later_filter()` directly, removing the local status set so the two can't drift again; the AC1 SQL oracle test was updated to encode the same "Stale only when a `quote_package_versions` row exists" rule independently, and a new `test_sent_or_later_matches_dashboard` cross-checks the tile count against `GET /api/v1/applications?status=sent_or_later`'s `total` for a Manager and for one LO. |
| 8 | Decision (stage-6 fix, cq-025-fix) | `LoOption` schema name collision with CQ-027 | Once CQ-027's router is also mounted, FastAPI/`openapi-typescript` had two distinct `LoOption` Pydantic models (this feature's and `applications.listing.schemas.LoOption`), so the OpenAPI component name fell back to the fully-qualified `app__features__..._LoOption`, breaking the `components["schemas"]["LoOption"]` re-export both frontends' `api.ts` relied on. Fixed: renamed this feature's schema to `DashboardLoOption` (backend `schemas.py`/`service.py` + frontend `dashboard/api.ts`'s re-export) — CQ-027's own `LoOption` name is unchanged. |
...
| AC4 | `e2e/lo-console/dashboard-tiles.spec.ts` (hrefs, plus "each Applications-linked tile opens the list with exactly the tile's count" now that CQ-027 is merged — cq-025-fix); `test_sent_or_later_matches_dashboard` (backend cross-check) |
