...
| --- | --- | --- | --- |
| 1 | Decision (revised in review) | Original spec pin literally contradicted itself: `FEATURE_ROUTERS` was pinned to contain `"app.features.system.router"` (which `register_routers` mounts under `/api/v1`), while the same paragraph also pinned `/health` as "not under `/api/v1`". The first pass here (correctly flagged by code review as a genuine spec contradiction, not a small gap to resolve unilaterally) resolved it by leaving both mounts in place, producing a duplicate `/api/v1/health`. | Orchestrator decision, applied here: `FEATURE_ROUTERS` starts **empty** (`system` is not a `/api/v1` feature); `create_app()` mounts `/health` **only** via the direct, unprefixed `app.include_router(system_router)` call. `spec.md`'s `registry.py` snippet and prose were updated to match (orchestrator-authorized edit — the one exception to "don't change `docs/design/*` or another item's spec.md", since this is our own item's spec resolving our own pin contradiction). `/api/v1/health` no longer exists, live or in `app.openapi()["paths"]`; `backend/tests/test_registry.py::test_health_is_unprefixed_only_not_also_under_api_v1` proves it both ways (live 404 and absent from the schema). |
...
| 11 | Decision (review finding #2) | `schemas.py`'s pinned `CheckResult` model was defined but never used — `HealthReport.checks` was built directly as `dict[str, str]` inside `run_health_checks`. | Decided: use `CheckResult` for real. Each `check_*` function in `service.py` now returns a `CheckResult(name=..., status=...)` instead of a bare string; `run_health_checks` flattens the four results into `HealthReport.checks`'s pinned `dict[str, str]` shape. The response JSON is byte-identical to the pinned example — `CheckResult` is an internal building block, not part of the wire shape. |
| 12 | Decision (review finding #3) | `make lint`'s mypy step only checked `backend/app`, leaving `backend/conftest.py`, `backend/tests/`, and `backend/scripts/` (a CQ-005 dependency) unchecked by the gate. | Decided: removed `packages = ["app"]` from `[tool.mypy]` (kept `mypy_path`/`disallow_untyped_defs`) and changed the Makefile's mypy line to `uv run mypy backend/app backend/conftest.py backend/tests backend/scripts`, so the whole `backend/` tree is gated. Verified clean: no new errors surfaced (the `conftest.py` `app` fixture was already typed `FastAPI`, not `object`, from the original PR). |
