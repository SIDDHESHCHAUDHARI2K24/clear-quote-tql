...
- A residual edge remains in `mark_stale`: an application that *becomes* a candidate between step 1 and step 3 has its quotes locked late. A reprice holding a lower-id quote of that application while waiting on an old one the job locked in step 1 could still cycle. This needs a status change committed mid-job; accepted and documented.
  - **U3 update (PR #35 review minor a):** the step-3 re-lock now covers only applications that were *not* candidates at step 1. A quote inserted mid-job into an application whose quotes step 1 already holds is no longer re-locked: it is fresh, so step 3's conditional UPDATE skips it without locking it. The edge above, a status change committed mid-job, is the only remaining late-lock case.
  - **U3, review minors b–d:** `test_mark_stale_locks_its_quotes_by_id_before_updating` now asserts the `ORDER BY quotes.id … FOR NO KEY UPDATE OF quotes` SQL and that it runs before the first quote UPDATE. The race tests no longer use a fixed `PAUSE`: they poll `pg_stat_activity` until the other backend waits on a lock, with a 10 s bound. SQL-order tests now cover Send `update_package` and the `get_or_create_package` slow path (packages → application), and the portal actions (package → version → application).
