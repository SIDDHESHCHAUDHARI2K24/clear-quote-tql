...
| 28 | Decision (028b) | Where occupancy/strategy live in the UI | `fields.py`'s `resolve_field` maps `occupancy_type`/`investment_strategy` to `ApplicationTab.PROPERTY`, and `service.py`'s `_property()` is the only tab builder that includes a `kind="loan"` record carrying those two fields (borrowers' records are party-only). 028b renders occupancy/investment strategy on the **Property** tab, inside a "Loan" card, not Borrowers — that is where the section API actually returns them. The AC1 E2E spec (`e2e/lo-console/aisha-occupancy-resume.spec.ts`) drives the fix from the Property tab. |
| 29 | Decision (028b) | One edit pattern for every `FieldRow` | `fields.py`'s `resolve_field` regexes cover application, party *and* row keys (`{collection}.{row_id}.{column}`) alike, so every `SectionField.field_key` the section API returns — whatever record it came from — can be edited/reverted through the generic `PUT`/`DELETE /fields/{field_key}` alone. 028b's `FieldRow` always calls `putField`/`revertField` with the field's own `field_key`; the row-collection endpoints (`addHousing`/`addParty`/`addLiability`, and the `PATCH .../housing_history/{row_id}` etc. the shared kit also wraps) are used only to add a new row or edit several of an existing row's columns in one request. This keeps every tab's edit code identical regardless of which collection a field belongs to. |
| 30 | Decision (028b) | Header TBD label (plan #19) | CQ-016's `ApplicationSummaryResponse` has no `address_status`/TBD field, and adding one is a backend change outside 028b's scope (028b does not touch backend files; a parallel hardening unit owns `sections/` this wave). 028b shows the TBD state (and clears it) inside the **Property tab** itself, next to the address (from `section.property.tbd`) — not in CQ-016's global sticky header. AC6's Playwright spec asserts the label there. Logged as a follow-up: CQ-016 (or a later item) should add `address_status` to the summary so the sticky header can show it too. |
...
- [x] T10 shared tab kit: `useSection`, `FieldRow`, `FlagList`, `runSectionAction`, api wrappers, `formatFieldValue`/`formatDtiRatio`
- [x] T11 Borrowers tab: cards, `SsnField` (10 s re-mask), add co-borrower, "No co-applicant"
- [x] T12 Housing tab: rows, total months vs 24, add prior address
- [x] T13 Credit tab: FICO/bracket, liabilities, DTI (primary only), import, hard pull, E11 consent states
- [x] T14 Assets tab: assets, reserves/sufficiency, employment/income (primary only), document checklist
- [x] T15 Property tab: address/TBD, occupancy+strategy (Loan card), recommend-matches toggle, two-tier buy-box picker
- [x] T16 react-doctor, Playwright, UI evidence in post-dev.md
