...
`make_application` pattern; `make_application`/`set_field_value` mirror
`pricing/conftest.py`'s versions so `test_settings_match_snapshot` (AC6)
can build a scenario through the real `create_default_scenarios` service
call instead of hand-assembling a `Scenario` row -- code review round 1,
minor 2)."""
