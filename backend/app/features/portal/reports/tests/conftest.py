"""Local fixtures for `portal/reports/tests` -- own copy of
`pricing/conftest.py`'s `make_application`/`set_field_value` pattern (not an
import: pytest's conftest discovery only reaches up the directory tree, and
`portal/reports/tests` is a sibling of `pricing/`, not nested under it --
same reasoning as `seed/tests/conftest.py`'s own docstring).
"""
...
async def make_application(
    ...
    async def _make(
        ...
        is_tbd = address_status == PropertyAddressStatus.TBD
        db_session.add(
            Property(
                application_id=application.id,
                address_status=address_status,
                street_address=None if is_tbd else street_address,
                state=state,
                county=county,
                zip=zip_code,
...
def seed_conventional_rate_sheet(db_session: AsyncSession) -> None:
    """A minimal Conventional curve so `auto_price` has par + buydown
    candidates for a PRIMARY application (matches
    `quotes/builder/tests/test_draft_quote_set.py`'s own fixture)."""
...
def seed_dscr_rate_sheet(db_session: AsyncSession, dscr_bucket: str, par_rate: Decimal) -> None:
    """A minimal DSCR curve for an investment (LTR/STR) application, at a
    single DSCR bucket -- matches
